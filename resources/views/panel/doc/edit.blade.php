...
@section('title', 'ویرایش مستند: ' . Str::limit($doc->title, 50))
            ...
            <div class="col-lg-12">
                <div class="card m-b-30 shadow-sm">
                    <div class="card-header">
                        <h5 class="card-title mb-0">ویرایش مستند: {{ Str::limit($doc->title, 60) }}</h5>
                    </div>
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        <form action="{{ route('doc.update', $doc->id) }}" method="POST" enctype="multipart/form-data" id="doc-form-edit">
                                    ...
                                    <select name="subject_id" class="form-control @error('subject_id') is-invalid @enderror" required>
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}"
                                                {{ old('subject_id', $doc->subject_id) == $subject->id ? 'selected' : '' }}>
                            ...
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    ...
                                    <small class="text-muted">حداکثر ۴ مگابایت – jpg, png, gif, webp</small>
                                    @error('image')
                                    <span class="text-danger mt-2 d-block">{{ $message }}</span>
                                    @enderror
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            ClassicEditor
                .create(document.querySelector('#editor-edit'), {
                    language: 'fa',
                    toolbar: {
                        items: ['heading','|','bold','italic','underline','|','link','bulletedList','numberedList','|','blockQuote','insertTable','codeBlock','|','undo','redo'],
